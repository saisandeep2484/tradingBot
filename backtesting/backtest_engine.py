...
def fetch_historical_data(brokerage_client, symbol, exchange, instrument_token):
    to_date = datetime.datetime.today()
    from_date = to_date - datetime.timedelta(days=DAYS)
    print(f"Fetching historical data for {symbol} ({exchange}) from {from_date.date()} to {to_date.date()}")

    data = brokerage_client.fetch_ohlc(symbol=symbol, exchange=exchange, interval=INTERVAL, duration=DAYS)
...
def main():
    ...
    if use_live_data:
        ...
        try:
            ...
            df = fetch_historical_data(brokerage_client, symbol, exchange, instrument_token)
        except Exception as e:
